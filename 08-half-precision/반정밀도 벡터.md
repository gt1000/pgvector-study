# 08. 반정밀도 벡터 (Half-Precision Vectors)

메모리 사용량을 절반으로 줄이는 halfvec 타입 활용법

> 🎯 **이 문서의 목표:**  
> 저장 공간과 성능을 최적화하면서 높은 정확도를 유지하는 방법 학습

---

## 1) halfvec 타입 개요
### halfvec란?

**halfvec**는 16비트 부동소수점(float16)을 사용하는 벡터 타입입니다.

**기본 비교:**

| 타입 | 비트 수 | 차원당 크기 | 768차원 크기 | 정밀도 |
|------|--------|-----------|------------|--------|
| **vector** | 32bit (float32) | 4 bytes | 3,072 bytes (3KB) | 높음 |
| **halfvec** | 16bit (float16) | 2 bytes | 1,536 bytes (1.5KB) | 중간 |
| **bit** | 1bit | 0.125 bytes | 96 bytes | 낮음 |

**핵심:**
- 저장 공간: **50% 절약** ✅
- 검색 속도: 약간 빠름 (데이터 전송량 감소)
- 정확도: 95~99% 유지 (용도에 따라 다름)

---

### 언제 사용하나?

```
✅ 수백만 개 이상 대용량 벡터 저장
✅ 메모리/디스크 공간 제약
✅ 95% 정확도로 충분한 경우
✅ 검색 속도 최적화 필요
❌ 매우 높은 정확도 필요 (과학 계산 등)
❌ 데이터가 적음 (1만 건 미만)
```

---

### float16 정밀도

**표현 범위:**
```
float32: ±3.4 × 10³⁸ (약 7자리 정밀도)
float16: ±65,504 (약 3자리 정밀도)

예시:
float32: 0.123456789
float16: 0.1235 (자동 반올림)
```

**주의:**
- 매우 큰 값이나 매우 작은 값은 표현 제한
- 임베딩 모델이 정규화된 벡터를 생성하면 문제없음

---

## 2) halfvec 테이블 생성

### ① 기본 테이블

```sql
-- halfvec 타입 컬럼
CREATE TABLE paper_chunks_half (
    id BIGSERIAL                PRIMARY KEY,
    paper_id TEXT               NOT NULL,
    chunk_index                 INT NOT NULL,
    chunk_text                  TEXT NOT NULL,
    embedding                   halfvec(768) NOT NULL,  -- halfvec 사용
    metadata                    JSONB DEFAULT '{}',
    created_at                  TIMESTAMP DEFAULT NOW(),
    UNIQUE(paper_id, chunk_index)
);
```

**vector vs halfvec 비교:**

```sql
-- vector (float32)
CREATE TABLE docs_full (
    id BIGSERIAL PRIMARY KEY,
    embedding vector(768) NOT NULL  -- 3KB per row
);

-- halfvec (float16)
CREATE TABLE docs_half (
    id BIGSERIAL PRIMARY KEY,
    embedding halfvec(768) NOT NULL  -- 1.5KB per row
);
```

---

### ② 저장 공간 비교

**1백만 행 기준:**

| 타입 | 행당 크기 | 총 크기 | 절약 |
|------|----------|---------|------|
| vector(768) | 3KB | 3GB | - |
| **halfvec(768)** | 1.5KB | **1.5GB** | **50%** ✅ |

**실제 계산:**

```sql
-- 테이블 크기 확인
SELECT 
    pg_size_pretty(pg_total_relation_size('paper_chunks_full')) AS full_size,
    pg_size_pretty(pg_total_relation_size('paper_chunks_half')) AS half_size;

-- 예상 결과:
-- full_size: 3.2GB (인덱스 포함)
-- half_size: 1.6GB (인덱스 포함)
```

---

## 3) halfvec 데이터 삽입

### ① 직접 삽입

```sql
-- 문자열로 삽입
INSERT INTO paper_chunks_half (paper_id, chunk_index, chunk_text, embedding)
VALUES (
    'paper_001',
    0,
    '청크 내용',
    '[0.1, 0.2, 0.3, ...]'::halfvec(768)  -- halfvec로 캐스팅
);
```

---

### ② vector → halfvec 변환

```sql
-- vector 타입에서 halfvec로 자동 변환
INSERT INTO paper_chunks_half (paper_id, chunk_index, chunk_text, embedding)
SELECT 
    paper_id,
    chunk_index,
    chunk_text,
    embedding::halfvec(768)  -- vector를 halfvec로 변환
FROM paper_chunks_full;
```

**변환 예시:**

```sql
-- 원본 (vector)
SELECT embedding FROM paper_chunks_full LIMIT 1;
-- [0.123456789, -0.987654321, ...]

-- 변환 후 (halfvec)
SELECT embedding FROM paper_chunks_half LIMIT 1;
-- [0.1235, -0.9876, ...]  (정밀도 감소)
```

---

### ③ Python에서 삽입

```python
from sentence_transformers import SentenceTransformer
import psycopg2
import numpy as np

model = SentenceTransformer("jhgan/ko-sbert-sts")
conn = psycopg2.connect(...)
cur = conn.cursor()

# 임베딩 생성 (float32)
text = "논문 청크 내용"
embedding_float32 = model.encode(text)  # numpy array (float32)

# halfvec로 삽입 (PostgreSQL이 자동 변환)
cur.execute("""
    INSERT INTO paper_chunks_half (paper_id, chunk_index, chunk_text, embedding)
    VALUES (%s, %s, %s, %s::halfvec(768))
""", ('paper_001', 0, text, embedding_float32.tolist()))

conn.commit()
```

**중요:** Python에서는 float32 그대로 보내고, PostgreSQL이 halfvec로 변환합니다.

---

### ④ 대량 삽입

```python
from psycopg2.extras import execute_batch

# 대량 데이터 준비
data = []
for i, text in enumerate(texts):
    embedding = model.encode(text).tolist()
    data.append(('paper_001', i, text, embedding))

# Batch Insert
execute_batch(cur, """
    INSERT INTO paper_chunks_half (paper_id, chunk_index, chunk_text, embedding)
    VALUES (%s, %s, %s, %s::halfvec(768))
""", data, page_size=1000)

conn.commit()
```

---

## 4) halfvec 검색

### ① 기본 검색

```sql
-- Cosine Distance 검색
SELECT 
    chunk_text,
    embedding <=> '[0.1, 0.2, ...]'::halfvec(768) AS distance
FROM paper_chunks_half
WHERE paper_id = 'paper_001'
ORDER BY distance
LIMIT 10;
```

**지원 연산자:**

| 연산자 | 거리 함수 | halfvec 지원 |
|--------|----------|------------|
| `<=>` | Cosine Distance | ✅ |
| `<->` | L2 Distance | ✅ |
| `<#>` | Inner Product | ✅ |

---

### ② Python에서 검색

```python
# 질의 임베딩 생성 (float32)
question = "최적 수온은?"
query_embedding = model.encode(question).tolist()

# halfvec 테이블 검색 (자동 변환)
cur.execute("""
    SELECT 
        chunk_text,
        embedding <=> %s::halfvec(768) AS distance
    FROM paper_chunks_half
    WHERE paper_id = %s
    ORDER BY distance
    LIMIT 5
""", (query_embedding, 'paper_001'))

results = cur.fetchall()
```

---

### ③ vector ↔ halfvec 혼용

```sql
-- vector 쿼리로 halfvec 테이블 검색 (자동 변환)
SELECT 
    chunk_text,
    embedding <=> '[0.1, 0.2, ...]'::vector(768) AS distance
FROM paper_chunks_half
ORDER BY distance
LIMIT 10;

-- halfvec 쿼리로 vector 테이블 검색 (자동 변환)
SELECT 
    chunk_text,
    embedding <=> '[0.1, 0.2, ...]'::halfvec(768) AS distance
FROM paper_chunks_full
ORDER BY distance
LIMIT 10;
```

PostgreSQL이 자동으로 타입 변환을 처리합니다.

---

## 5) halfvec 인덱싱

### ① HNSW 인덱스

```sql
-- Cosine Distance 인덱스
CREATE INDEX idx_half_embedding_cosine
ON paper_chunks_half
USING hnsw (embedding halfvec_cosine_ops)  -- halfvec 전용 ops
WITH (m = 16, ef_construction = 64);

-- L2 Distance 인덱스
CREATE INDEX idx_half_embedding_l2
ON paper_chunks_half
USING hnsw (embedding halfvec_l2_ops)
WITH (m = 16, ef_construction = 64);

-- Inner Product 인덱스
CREATE INDEX idx_half_embedding_ip
ON paper_chunks_half
USING hnsw (embedding halfvec_ip_ops)
WITH (m = 16, ef_construction = 64);
```

**Operator Class:**

| 검색 연산자 | Operator Class | 타입 |
|-----------|---------------|------|
| `<=>` | `halfvec_cosine_ops` | halfvec |
| `<->` | `halfvec_l2_ops` | halfvec |
| `<#>` | `halfvec_ip_ops` | halfvec |

---

### ② IVFFlat 인덱스

```sql
-- IVFFlat 인덱스 (대용량)
CREATE INDEX idx_half_embedding_ivf
ON paper_chunks_half
USING ivfflat (embedding halfvec_cosine_ops)
WITH (lists = 100);
```

---

### ③ 인덱스 크기 비교

**1백만 행 기준:**

| 타입 | 인덱스 크기 | 절약 |
|------|-----------|------|
| vector(768) + HNSW | ~2GB | - |
| **halfvec(768) + HNSW** | **~1GB** | **50%** ✅ |

```sql
-- 인덱스 크기 확인
SELECT 
    indexname,
    pg_size_pretty(pg_relation_size(indexname::regclass)) AS size
FROM pg_indexes
WHERE tablename IN ('paper_chunks_full', 'paper_chunks_half');
```

---

## 6) 성능 비교

### ① 검색 속도

**테스트 환경:** 1백만 행, 768차원

| 타입 | 인덱스 | 검색 시간 | 정확도 |
|------|-------|----------|--------|
| vector | HNSW | 12ms | 99% |
| **halfvec** | HNSW | **10ms** | 97% |
| vector | 없음 | 520ms | 100% |
| halfvec | 없음 | 480ms | 100% |

**관찰:**
- halfvec가 약간 빠름 (데이터 전송량 50% 감소)
- 정확도는 2% 감소 (대부분 용도에 충분)

---

### ② 메모리 사용량

```sql
-- 메모리 사용량 비교
SELECT 
    schemaname,
    tablename,
    pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) AS total_size
FROM pg_tables
WHERE tablename IN ('paper_chunks_full', 'paper_chunks_half');
```

**예상 결과 (1백만 행):**

| 테이블 | 데이터 | 인덱스 | 총합 |
|--------|--------|--------|------|
| paper_chunks_full | 3.0GB | 2.0GB | **5.0GB** |
| paper_chunks_half | 1.5GB | 1.0GB | **2.5GB** |

---

### ③ 정확도 테스트

```python
import numpy as np
from sentence_transformers import SentenceTransformer

model = SentenceTransformer("jhgan/ko-sbert-sts")

# 테스트 데이터
texts = ["문서1", "문서2", "문서3", ...]
query = "검색 질의"

# float32 임베딩
embeddings_f32 = model.encode(texts)
query_f32 = model.encode(query)

# float16 변환
embeddings_f16 = embeddings_f32.astype(np.float16)
query_f16 = query_f32.astype(np.float16)

# Cosine 유사도 계산
from sklearn.metrics.pairwise import cosine_similarity

sim_f32 = cosine_similarity([query_f32], embeddings_f32)[0]
sim_f16 = cosine_similarity([query_f16], embeddings_f16)[0]

# 상위 10개 비교
top10_f32 = np.argsort(sim_f32)[-10:][::-1]
top10_f16 = np.argsort(sim_f16)[-10:][::-1]

# 일치율 계산
overlap = len(set(top10_f32) & set(top10_f16))
print(f"정확도: {overlap}/10 = {overlap*10}%")
# 일반적으로 8~10/10 = 80~100%
```

---

## 7) vector → halfvec 마이그레이션

### ① 기존 테이블 변환

```sql
-- Step 1: halfvec 테이블 생성
CREATE TABLE paper_chunks_half (
    id BIGSERIAL PRIMARY KEY,
    paper_id TEXT NOT NULL,
    chunk_index INT NOT NULL,
    chunk_text TEXT NOT NULL,
    embedding halfvec(768) NOT NULL,
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMP DEFAULT NOW(),
    UNIQUE(paper_id, chunk_index)
);

-- Step 2: 데이터 복사 (자동 변환)
INSERT INTO paper_chunks_half
SELECT 
    id,
    paper_id,
    chunk_index,
    chunk_text,
    embedding::halfvec(768),  -- 변환
    metadata,
    created_at
FROM paper_chunks_full;

-- Step 3: 인덱스 생성
CREATE INDEX idx_half_embedding
ON paper_chunks_half
USING hnsw (embedding halfvec_cosine_ops);

-- Step 4: 확인
SELECT COUNT(*) FROM paper_chunks_half;
```

---

### ② 컬럼 타입 변경 (비추천)

```sql
-- 주의: 시간이 매우 오래 걸림 (테이블 재작성)
ALTER TABLE paper_chunks_full
ALTER COLUMN embedding TYPE halfvec(768);

-- 대량 데이터에서는 새 테이블 생성 권장
```

---

### ③ 점진적 마이그레이션

```sql
-- 배치로 나눠서 복사 (대용량)
DO $$
DECLARE
    batch_size INT := 10000;
    offset_val INT := 0;
    total INT;
BEGIN
    SELECT COUNT(*) INTO total FROM paper_chunks_full;
    
    WHILE offset_val < total LOOP
        INSERT INTO paper_chunks_half
        SELECT 
            id,
            paper_id,
            chunk_index,
            chunk_text,
            embedding::halfvec(768),
            metadata,
            created_at
        FROM paper_chunks_full
        LIMIT batch_size OFFSET offset_val;
        
        offset_val := offset_val + batch_size;
        RAISE NOTICE '진행: % / %', offset_val, total;
        COMMIT;
    END LOOP;
END $$;
```

---

## 8) 실전 사용 패턴

### ① 하이브리드 전략

```sql
-- 자주 검색: halfvec (빠름)
CREATE TABLE hot_chunks (
    id BIGSERIAL PRIMARY KEY,
    chunk_text TEXT,
    embedding halfvec(768)
);

-- 보관용: vector (정확)
CREATE TABLE cold_chunks (
    id BIGSERIAL PRIMARY KEY,
    chunk_text TEXT,
    embedding vector(768)
);
```

---

### ② Two-Tier 검색

```sql
-- 1단계: halfvec로 빠르게 후보 추출
WITH candidates AS (
    SELECT id
    FROM paper_chunks_half
    ORDER BY embedding <=> '[...]'::halfvec(768)
    LIMIT 100
)
-- 2단계: vector로 정밀 재정렬
SELECT 
    p.chunk_text,
    p.embedding <=> '[...]'::vector(768) AS distance
FROM paper_chunks_full p
JOIN candidates c ON p.id = c.id
ORDER BY distance
LIMIT 10;
```

---

## 9) 주의사항

### ① 정밀도 손실

```
정밀도 손실이 큰 경우:
❌ 과학 계산 (높은 정밀도 필요)
❌ 금융 데이터 (정확한 값 필요)
⚠️ 매우 작은 차이 비교

정밀도 손실이 작은 경우:
✅ 텍스트 검색 (의미 유사도)
✅ 이미지 검색
✅ 추천 시스템
```

---

### ② 값 범위 제한

```sql
-- float16 범위: ±65,504
-- 범위 초과 시 Infinity로 변환됨

-- 테스트
SELECT '[70000]'::halfvec(1);
-- 결과: [Infinity]

-- 해결: 정규화된 임베딩 사용 (대부분 -1 ~ 1 범위)
```

---

### ③ 타입 혼용 주의

```sql
-- ❌ 잘못된 예: operator class 불일치
CREATE INDEX idx_wrong
ON paper_chunks_half
USING hnsw (embedding vector_cosine_ops);  -- vector ops 사용

-- ✅ 올바른 예: halfvec ops 사용
CREATE INDEX idx_correct
ON paper_chunks_half
USING hnsw (embedding halfvec_cosine_ops);  -- halfvec ops
```

---

## 10) 선택 가이드

| 상황 | 권장 타입 | 이유 |
|------|----------|------|
| **1만 건 미만** | vector | 공간 절약 효과 미미 |
| **10만 ~ 100만 건** | halfvec | 50% 공간 절약 ✅ |
| **100만 건 이상** | halfvec | 필수 (메모리/비용) ✅ |
| **높은 정확도 필요** | vector | 99%+ 정확도 |
| **95% 정확도 충분** | halfvec | 빠르고 효율적 |
| **메모리 제약** | halfvec | 50% 절약 ✅ |


```
halfvec = float16 = 16bit

장점:
✅ 저장 공간 50% 절약
✅ 검색 속도 약간 빠름
✅ 인덱스 크기 50% 절약
✅ 95~99% 정확도 유지

단점:
⚠️ 정밀도 약간 감소 (3자리)
⚠️ 값 범위 제한 (±65,504)
```
---
